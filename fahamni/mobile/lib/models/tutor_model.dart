  ...
  factory TutorModel.fromMap(Map<String, dynamic> map) {
    ...
    Gender parseGender(dynamic value) {
      ...
      for (final Gender gender in Gender.values) {
        if (gender.name == normalized) return gender;
      }
      return Gender.male;
    }
    ...
    return TutorModel(
      ...
      expertiseDomain: (map['expertise_domain'] ?? map['expertiseDomain'] ?? '').toString(),
      levelsTaught: List<String>.from(map['levels_taught'] ?? map['levelsTaught'] ?? const <String>[]),
      teachingMode: (map['teaching_mode'] ?? map['teachingMode'] ?? '').toString(),
      isAvailable: map['is_available'] ?? map['isAvailable'] ?? false,
      certified: map['certified'] ?? map['is_certified'] ?? map['isVerified'] ?? false,
      pedagogicalDescription: (map['pedagogical_description'] ?? map['pedagogicalDescription'] ?? '').toString(),
      averageRating: parseDouble(map['average_rating'] ?? map['averageRating']),
      yearsOfExperience: parseInt(map['years_of_experience'] ?? map['yearsOfExperience']),
      academicDescription: (map['academic_description'] ?? map['academicDescription'] ?? '').toString(),
      studentsCount: parseInt(map['students_count'] ?? map['studentsCount']),
      coursesCount: parseInt(map['courses_count'] ?? map['coursesCount']),
      certificationUrl: (map['certification_url'] ?? '').toString(),
