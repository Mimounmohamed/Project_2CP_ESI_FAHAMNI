...
class _ParentSchedulePageState extends State<ParentSchedulePage> {
  ...
  Future<_ParentScheduleViewData> _loadSchedule({
    ...
    if (selectedChild == null) {
      return _ParentScheduleViewData(
        children: children,
        selectedChild: null,
        sessions: const <_ScheduledSession>[],
      );
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: SafeArea(
        child: FutureBuilder<_ParentScheduleViewData>(
                ...
                Expanded(
                  child: data.sessions.isEmpty
                      ? _ScheduleMessageState(
                          ...
                          subtitle:
                              'Booked sessions will appear here in agenda form.',
                          actionLabel: 'Refresh',
                          onAction: _refresh,
                        )
                      : _buildModeBody(data.sessions),
                ),
              ],
            );
          },
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        final int selectedIndex = days
            .indexWhere(
              (day) => _ParentSchedulePageState._isSameDay(day, selectedDay),
            )
              ...
              children: [
                ...
                ...days.asMap().entries.map((entry) {
                  ...
                  final List<_ScheduledSession> daySessions =
                      sessions
                          .where(
                            (item) => _ParentSchedulePageState._isSameDay(
                              _ParentSchedulePageState._sessionStart(
                                item.session,
                              ),
                              day,
                            ),
                  ...
                  return Positioned(
                    ...
                    child: Stack(
                      children: daySessions.asMap().entries.map((itemEntry) {
                        ...
                        final DateTime end =
                            _ParentSchedulePageState._sessionEnd(item.session);
                        final double top =
        ...
        Expanded(
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final bool selected = _ParentSchedulePageState._isSameDay(
                day,
                focusedDate,
              );
              final List<_ScheduledSession> daySessions = sessions
                  .where(
                    (item) => _ParentSchedulePageState._isSameDay(
                      _ParentSchedulePageState._sessionStart(item.session),
                      day,
                    ),
                  )
                  .toList();
...
class _ParentScheduleViewData {
  const _ParentScheduleViewData({
    required this.children,
    required this.selectedChild,
    required this.sessions,
  });
